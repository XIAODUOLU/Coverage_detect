...
def main(cfg, args):
    ...
    model = get_reload_weight_simple(model, pth='./mydata_par_api.path')                               # 修改此处的模型名字
    model.eval()
...
def detect_par_api(cfg_path, img, nattrnum, modelselectpath):
    ...
    model = get_reload_weight_simple(model, pth= modelselectpath)            # 修改此处的模型名字
    model.eval()
...
def argument_parser():
    ...
    parser.add_argument(
        "--cfg", help="decide which cfg to use", type=str, default='./mydata.yaml'
    )
